...
# Загружаем .env локально (в CI файла нет — просто ничего не сделает)
load_dotenv()


class Config:
    # ===== Базовые настройки =====
    # BASE_URL не секрет — дефолт для локального запуска
    BASE_URL = os.getenv("BASE_URL", "https://manufacture.lar.tech")

    # Логин/пароль — секреты. В CI приходят из GitHub Secrets, локально — из .env
    TEST_USERNAME = os.getenv("TEST_USERNAME")
    TEST_PASSWORD = os.getenv("TEST_PASSWORD")

    # Раннее падение с понятной ошибкой, если секреты не заданы
    if not TEST_USERNAME or not TEST_PASSWORD:
        raise RuntimeError(
            "TEST_USERNAME / TEST_PASSWORD не заданы. "
            "В CI — добавь secrets, локально — создай .env"
        )
